...
class RobotInspector(object):
    ...
    def __init__(self, robosimian, RobotUtils, MotionPlanner):
    ...
    def print_config(self):
        print self.RobotUtils.pp_list(self.robosimian.getConfig())
    ...
    def update_torso_COM(self):
        ...
        center = [x, y, z]
        ground = [x, y, z + self.RobotUtils.BASE_STATE_Z_DELTA]
        self.add_line_to_vis("torso xy", center, ground)

    def get_lr_distance(self):
        fl = self.MotionPlanner.get_end_affector_from_end_affector_name(self.RobotUtils.F_L_FOOT)
        fr = self.MotionPlanner.get_end_affector_from_end_affector_name(self.RobotUtils.F_R_FOOT)
        ...
        bs_y_delta = self.RobotUtils.BASE_STATE_Y_DELTA
        shoulder_y = self.RobotUtils.SHOULDER_Y
        sum_y       = bs_y_delta + shoulder_y

        print "\nfl, fr  world:\t",self.RobotUtils.pp_list(fl_xyz_w), "\t", self.RobotUtils.pp_list(fr_xyz_w)
        print "fl, fr  local:\t",self.RobotUtils.pp_list(fl_xyz_l), "\t", self.RobotUtils.pp_list(fr_xyz_l)
        print "delta xyz world:",self.RobotUtils.pp_list(delta_xyz_w)
        print "delta xyz local:",self.RobotUtils.pp_list(delta_xyz_l)
        print "base state y delta, shoulder y, sum, 2x sum:",self.RobotUtils.pp_double(bs_y_delta),self.RobotUtils.pp_double(shoulder_y),self.RobotUtils.pp_double(sum_y),self.RobotUtils.pp_double(2*sum_y),"\n"


    def get_fb_distance(self):

        fl = self.MotionPlanner.get_end_affector_from_end_affector_name(self.RobotUtils.F_L_FOOT)
        bl = self.MotionPlanner.get_end_affector_from_end_affector_name(self.RobotUtils.B_L_FOOT)
        ...
        bs_x_delta = self.RobotUtils.BASE_STATE_X_DELTA
        shoulder_x = self.RobotUtils.SHOULDER_X
        sum_x = bs_x_delta + shoulder_x

        print "\nfl, bl  world:\t", self.RobotUtils.pp_list(fl_xyz_w), "\t", self.RobotUtils.pp_list(bl_xyz_w)
        print "fl, bl  local:\t", self.RobotUtils.pp_list(fl_xyz_l), "\t",self.RobotUtils.pp_list(bl_xyz_w)
        print "delta xyz world:", self.RobotUtils.pp_list(delta_xyz_w)
        print "delta xyz local:", self.RobotUtils.pp_list(delta_xyz_l)
        print "base state x delta, shoulder x, sum, 2x sum:", self.RobotUtils.pp_double(bs_x_delta), self.RobotUtils.pp_double(shoulder_x), self.RobotUtils.pp_double(sum_x), self.RobotUtils.pp_double(2 * sum_x),"\n"
